...
static void find_available_ifaces()
{
	...
	getifaddrs(&addrs);  // 获取系统所有网络接口信息
	for (addr = addrs; addr != NULL; addr = addr->ifa_next) {
		if (addr->ifa_addr && addr->ifa_addr->sa_family == AF_PACKET && \
				strstr(addr->ifa_name, "-eth") != NULL) // 有地址信息，是数据链路层接口，并且名字中包含“-eth”
		{
	...
	iface_info_t *iface = NULL;
	list_for_each_entry(iface, &instance->iface_list, list) {
		sprintf(dev_names + strlen(dev_names), " %s", iface->name); // 拼接接口名字
	}
...
void init_ustack() // 初始化全局的 ustack 结构体
{
	instance = malloc(sizeof(ustack_t));  // 分配内存给 instance

	bzero(instance, sizeof(ustack_t));  // 清零
	init_list_head(&instance->iface_list);  // 初始化接口链表头

	init_all_ifaces();  // 初始化所有接口
} 
